...
function preload(){
  ...
  cooked_ground_nibbleaf = loadImage('ingredients/nibbleaf_ground_cooked.png');
  ...
  cooked_ground_wailotte = loadImage('ingredients/wailotte_ground_cooked.png');
  //dishes
  ...
  stew = loadImage('dishes/stew.png');
...
function render_item(x,y,w,player){
  ...
  if (player.item != "none"){
    ...
    if (player.item == "cubloaf"){
      ...
      image(cooked_diced_wailotte,x-w*0.9,y-w*4.4,w*1.7,w*1.7);
    }
      ...
      image(cooked_ground_nibbleaf,x-w*0.8,y-w*4.4,w*1.6,w*1.6);
    }
      ...
      image(cooked_ground_wailotte,x-w*0.8,y-w*4.4,w*1.6,w*1.6);
    }
  }
}
...
function draw() {
  ...
  if (run_once_bool){
    recepie_images.push(curry,skewers,jiggly_burger,classic_burger,nibble_springs_salad,Bouchers_casserole,stew)
    run_once_bool = false
  }
    //reset delta values
  player1.dx = 0
